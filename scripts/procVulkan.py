...
VULKAN_SDK_INSTALLER_URL = 'https://sdk.lunarg.com/sdk/download/1.4.341.1/windows/vulkansdk-windows-X64-1.4.341.1.exe'
UGE_VULKAN_VERSION = '1.4.341.1'
VULKAN_SDK_PATH = 'Uge/thirdparty/VulkanSDK'
VULKAN_SDK_EXE_PATH = 'Uge/thirdparty/VulkanSDK/VulkanSDK.exe'

def InstallVulkanSDK():
    if not Path(VULKAN_SDK_PATH).is_dir():
        os.mkdir(VULKAN_SDK_PATH)
...
def CheckVulkanSDK():
    if (VULKAN_SDK is None):
        ...
        InstallVulkanPrompt()
        return False
    elif (UGE_VULKAN_VERSION not in VULKAN_SDK):
        print(f"Located Vulkan SDK at {VULKAN_SDK}")
        print(f"You don't have the correct Vulkan SDK version! (Uge requires {UGE_VULKAN_VERSION})")
        InstallVulkanPrompt()
        return False
...
OutputDirectory = "Uge/thirdparty/VulkanSDK"
